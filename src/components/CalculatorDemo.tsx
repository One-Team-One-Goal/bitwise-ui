...
import { calculatorService, type SimplificationResult, type TruthTableResult } from '../services/calculator.service';

interface CalculatorDemoProps {
  className?: string;
}

export const CalculatorDemo: React.FC<CalculatorDemoProps> = ({ className }) => {
  const [expression, setExpression] = useState('A ∧ B ∨ ¬A');
  const [result, setResult] = useState<any>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [operation, setOperation] = useState<'simplify' | 'evaluate' | 'truthTable'>('simplify');
  const [variables, setVariables] = useState<Record<string, boolean>>({ A: true, B: false });
  ...
  const renderSimplificationResult = (result: SimplificationResult) => (
    <div className="space-y-4">
      <div>
        ...
        <p className="font-mono bg-gray-100 p-2 rounded">{result.originalExpression}</p>
      </div>
      
      <div>
        <h3 className="font-semibold">Simplified Expression:</h3>
        <p className="font-mono bg-green-100 p-2 rounded">{result.simplifiedExpression}</p>
      </div>
        ...
        <div className="space-y-2">
          {result.steps.map((step, index) => (
            <div key={index} className="border-l-4 border-blue-200 pl-4">
              ...
              <div className="text-xs text-gray-600">{step.lawName} ({step.law})</div>
            </div>
          ))}
        </div>
      </div>
            ...
            <thead>
              <tr className="bg-gray-100">
                {result.variables.map((variable) => (
                  <th key={variable} className="border border-gray-300 px-4 py-2">
                    {variable}
                  </th>
                ))}
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        ...
        <div className="space-y-4 mb-6">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              Supported operators: ∧ (AND), ∨ (OR), ¬ (NOT), ⊕ (XOR), → (IMPLIES), ↔ (IFF)
            </p>
          </div>
          ...
          {operation === 'evaluate' && (
            <div>
              <label className="block text-sm font-medium mb-2">Variable Values:</label>
              <div className="space-y-2">
                  ...
                  <div key={variable} className="flex items-center space-x-2">
                    ...
                    <label className="flex items-center">
                      <input
                        ...
                        name={variable}
                        checked={value === true}
                        onChange={() => setVariables(prev => ({ ...prev, [variable]: true }))}
                        className="mr-1"
                      />
                      True
                    </label>
                    <label className="flex items-center">
                      <input
                        ...
                        name={variable}
                        checked={value === false}
                        onChange={() => setVariables(prev => ({ ...prev, [variable]: false }))}
                        className="mr-1"
                      />
                      False
                    </label>
                ...
                <button
                  type="button"
                  onClick={() => setVariables(prev => ({ ...prev, [String.fromCharCode(65 + Object.keys(prev).length)]: false }))}
            ...
            {operation === 'evaluate' && (
              <div>
                ...
                <p className="text-2xl font-mono">
                  <span className={`px-3 py-1 rounded ${result ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                    {result ? 'TRUE' : 'FALSE'}
                  </span>
                </p>
              </div>
        ...
        <div className="border-t pt-6 mt-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
            {Object.entries(calculatorService.getSupportedOperators()).map(([name, symbols]) => (
