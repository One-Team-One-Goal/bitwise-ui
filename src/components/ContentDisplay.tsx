...
const ContentDisplay: React.FC<ContentDisplayProps> = ({
              ...
              {block.table?.headers.map((header, i) => (
                <th
                  ...
                  className={`px-6 py-3 text-sm font-bold text-gray-900 dark:text-gray-100 border-r border-gray-200 dark:border-gray-700 last:border-r-0 ${
                    block.table?.highlightCols?.includes(i) ? 'bg-yellow-100 dark:bg-yellow-900/30' : ''
                  }`}
                >
            ...
            {block.table?.rows.map((row, rIdx) => (
              <tr
                ...
                className={`border-b border-gray-100 dark:border-gray-800 last:border-b-0 ${
                  rIdx % 2 === 0 ? 'bg-white dark:bg-gray-900' : 'bg-gray-50 dark:bg-gray-800/50'
                ...
                {row.map((cell, cIdx) => (
                  <td
                    ...
                    className={`px-6 py-3 text-sm font-medium text-gray-900 dark:text-gray-100 border-r border-gray-100 dark:border-gray-800 last:border-r-0 ${
                      block.table?.highlightCols?.includes(cIdx) ? 'bg-yellow-50 dark:bg-yellow-900/20' : ''
                    }`}
                  >
            ...
            {block.truthTable?.rows.map((row, rIdx) => (
              ...
              >
                {row.map((cell, cIdx) => (
                  ...
                  >
                    {typeof cell === 'string' ? renderTruthTableWithOverline(cell) : cell}
                  </td>
                ))}
              </tr>
            ))}
  ...
  const colLabels = numVars >= 2 ? grayCode(Math.floor(numVars / 2)) : grayCode(numVars)
            ...
            <thead>
              <tr>
                <th className="px-4 py-2 text-xs font-semibold text-green-700 dark:text-green-300">
                  {variables.slice(0, Math.ceil(numVars / 2)).join('')} \ {variables.slice(Math.ceil(numVars / 2)).join('')}
                </th>
              ...
              {rowLabels.map((rowLabel, rIdx) => (
                <tr key={rIdx}>
                  ...
                  {colLabels.map((_, cIdx) => {
                    ...
                    const grouping = block.karnaughMap?.groupings?.find(g => g.cells.includes(cellIndex))
  ...
  const renderGateShape = (type: string) => {
    const baseProps = { fill: "none", stroke: "currentColor", strokeWidth: "2" };
    
    switch (type) {
      case 'AND':
        return (
          <svg viewBox="0 0 100 60" className="w-32 h-20">
            <path d="M 20 10 L 50 10 Q 80 10 80 30 Q 80 50 50 50 L 20 50 Z" {...baseProps} />
  ...
  return (
    <svg viewBox="0 0 100 60" className="w-32 h-20">
      ...
      case 'NAND':
        return (
          <svg viewBox="0 0 100 60" className="w-32 h-20">
            <path d="M 20 10 L 50 10 Q 75 10 75 30 Q 75 50 50 50 L 20 50 Z" {...baseProps} />
  ...
  const gateStyles = {
    AND: { color: 'blue', symbol: '∧', desc: 'Output is 1 only if all inputs are 1' },
    OR: { color: 'green', symbol: '∨', desc: 'Output is 1 if any input is 1' },
    NOT: { color: 'red', symbol: '¬', desc: 'Inverts the input' },
    NAND: { color: 'purple', symbol: '⊼', desc: 'NOT-AND: Output is 0 only if all inputs are 1' },
    NOR: { color: 'orange', symbol: '⊽', desc: 'NOT-OR: Output is 0 if any input is 1' },
    XOR: { color: 'yellow', symbol: '⊕', desc: 'Output is 1 if inputs are different' },
    XNOR: { color: 'pink', symbol: '⊙', desc: 'Output is 1 if inputs are the same' },
  };
      ...
      <div className="rounded-xl border-2 border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 p-6 shadow-lg">
        <div className="flex flex-col items-center gap-4">
          ...
          <div className="w-full grid grid-cols-2 gap-4">
            {block.logicGate?.inputs && block.logicGate.inputs.length > 0 && (
              <div>
                <p className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Inputs:</p>
    ...
    <div key={key} className={`mb-6 ${block.className || ''}`}>
      <div className="rounded-xl border-2 border-indigo-300 dark:border-indigo-700 bg-gradient-to-br from-indigo-50 to-white dark:from-indigo-900/20 dark:to-gray-900 p-6 shadow-lg">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h4 className="text-lg font-bold text-indigo-900 dark:text-indigo-100">Boolean Expression</h4>
        </div>
        ...
        <div className="space-y-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-4 border border-indigo-200 dark:border-indigo-800">
            <p className="text-xs font-semibold text-indigo-600 dark:text-indigo-400 mb-2">Original Expression:</p>
            <code className="text-lg font-mono text-indigo-900 dark:text-indigo-100 font-bold">
              {renderFormulaWithOverline(block.booleanExpression?.expression || '')}
            </code>
          </div>

          {block.booleanExpression?.simplified && (
            <div className="bg-green-50 dark:bg-green-900/20 rounded-lg p-4 border border-green-200 dark:border-green-800">
              <p className="text-xs font-semibold text-green-600 dark:text-green-400 mb-2">Simplified:</p>
              <code className="text-lg font-mono text-green-900 dark:text-green-100 font-bold">
                {renderFormulaWithOverline(block.booleanExpression.simplified)}
              </code>
            </div>
          )}

          {block.booleanExpression?.steps && block.booleanExpression.steps.length > 0 && (
            <div className="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
              <p className="text-xs font-semibold text-gray-600 dark:text-gray-400 mb-3">Simplification Steps:</p>
...
const renderGateInCircuit = (gate: any, x: number, y: number) => {
  ...
  const renderGateShape = (type: string, centerX: number, centerY: number) => {
    const baseProps = { fill: "none", stroke: "#111827", strokeWidth: "1.8" };
    ...
    switch (type) {
      case 'AND':
        return (
          <g transform={`translate(${centerX}, ${centerY}) scale(${scale})`}>
            <path d="M -25 -15 L 0 -15 Q 25 -15 25 0 Q 25 15 0 15 L -25 15 Z" {...baseProps} fill="white" />
          </g>
        );
      case 'OR':
        return (
          <g transform={`translate(${centerX}, ${centerY}) scale(${scale})`}>
            ...
            <path d="M -25 -15 Q 5 -15 20 0 Q 5 15 -25 15" {...baseProps} fill="white" />
      ...
      case 'NOT':
        return (
          <g transform={`translate(${centerX}, ${centerY}) scale(${scale})`}>
            <path d="M -20 -15 L -20 15 L 15 0 Z" {...baseProps} fill="white" />
      ...
      case 'NAND':
        return (
          <g transform={`translate(${centerX}, ${centerY}) scale(${scale})`}>
            <path d="M -25 -15 L 0 -15 Q 20 -15 20 0 Q 20 15 0 15 L -25 15 Z" {...baseProps} fill="white" />
  ...
  // Parse circuit data
  const circuitData = block.circuitDiagram;
  if (!circuitData?.inputs || !circuitData?.gates) {
    return (
      <div key={key} className="mb-6 p-4 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-300 dark:border-red-700">
        <p className="text-red-800 dark:text-red-200">Invalid circuit data</p>
      </div>
    );
  }
  ...
  const finalOutput = circuitData.finalOutput || gates[gates.length - 1]?.output;
          ...
          >
            ...
            {gates.map((gate: any) =>
              gate.inputs.map((inputSignal: string, inputIdx: number) =>
                drawConnection(inputSignal, gate.id, inputIdx, gate.inputs.length)
              )
            )}

            {/* Draw connection from last gate to final output */}
            {finalOutput && signalSource[finalOutput] && 
    ...
    <div key={key} className={`mb-6 text-center ${block.className || ''}`}>
      ...
      case 'callout':
        const calloutStyles: Record<string, string> = {
          ...
          warning: 'bg-yellow-50 border-yellow-300 text-yellow-900 dark:bg-yellow-900/20 dark:border-yellow-700 dark:text-yellow-100',
          tip: 'bg-green-50 border-green-300 text-green-900 dark:bg-green-900/20 dark:border-green-700 dark:text-green-100',
          important: 'bg-red-50 border-red-300 text-red-900 dark:bg-red-900/20 dark:border-red-700 dark:text-red-100',
        }
        ...
        return (
          <div key={key} className={`mb-6 ${block.className || ''}`}>
            <div className={`border-2 rounded-xl p-5 shadow-sm ${calloutStyles[block.callout?.type || 'info']}`}>
              <div className="flex items-start space-x-3">
                <span className="text-2xl flex-shrink-0">{calloutIcons[block.callout?.type || 'info']}</span>
                <div className="flex-1">
                  {block.callout?.title && (
                    <h4 className="font-bold text-lg mb-2">{block.callout.title}</h4>
                  )}
                  <p className="text-sm leading-relaxed">{block.callout?.content}</p>
                </div>
              </div>
            </div>
          </div>
